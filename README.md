...
The griffe rendering check needs no setup on the default (unpinned) path: the action installs `docvet[griffe]`, so every check the `checks` input offers is available. Pinning `docvet-version` installs plain `docvet` and leaves the rendering check skipped — every release before 1.15.2 declares the extra as `griffe>=1.0,<2`, which now resolves to a hollow shim rather than a working griffe — so a pinned run has to install griffe itself.
