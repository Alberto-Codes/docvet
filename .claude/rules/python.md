...
Use full package paths and avoid relative imports. Import modules, not individual classes/functions (exception: typing and collections.abc modules).
...
Use maximum line length of **88 characters** (formatter target) with linter threshold of **100 characters**.
...
Use `from __future__ import annotations` at the top of every file. Do not annotate `self` or `cls` parameters.
...
Use implicit boolean evaluation for containers. Always use explicit `is` or `is not` for None checks.

## Functions and Methods

Keep functions focused and reasonably short (consider breaking up if exceeds ~40 lines). Never use mutable objects as default arguments.
...
Use list/dict/set comprehensions for simple transformations. Keep comprehensions simple and readable. Document generator functions with "Yields:" section in docstring.
