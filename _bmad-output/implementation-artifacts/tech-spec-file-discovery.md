...
**In Scope:**
- `discover_files()` public function accepting `DocvetConfig` + `DiscoveryMode` + optional explicit file list
- Four modes: DIFF (git diff), STAGED (git diff --cached), ALL (git ls-files scoped to src_root, rglob fallback), FILES (explicit paths)
...
| File | Purpose |
| ---- | ------- |
| `src/docvet/cli.py:16-23` | `DiscoveryMode` enum definition (DIFF, STAGED, ALL, FILES) |
| `src/docvet/cli.py:64-93` | `_resolve_discovery_mode()` — mutual exclusivity enforcement |
| `src/docvet/config.py:41-58` | `DocvetConfig` dataclass — `src_root`, `exclude`, `project_root` fields |
| `src/docvet/config.py:127-136` | `_validate_keys()` — stderr warning pattern to follow |
| `tests/integration/conftest.py:10-26` | `git_repo` fixture — `tmp_path` with `git init` + user config |
| `tests/unit/test_config.py:25-31` | Factory fixture pattern (`write_pyproject`) to emulate |
| `tests/unit/test_cli.py:11-17` | `CliRunner` + `_strip_ansi` helper pattern |
| `pyproject.toml:75-87` | Pytest config — `--strict-markers`, registered markers |
...
4. **`--diff-filter=ACMR`** — Excludes deleted files (Added, Copied, Modified, Renamed)
...
9. **No symlink following** — Industry standard (ruff, black, ty). Prevents infinite loops from circular symlinks. Use `Path.is_symlink()` check in `_walk_all` fallback.
...
12. **Single public API** — `discover_files()` is the only public function. Helpers `_run_git()`, `_is_excluded()`, `_walk_all()` are private.
...
|---------|-----------------------------------|--------|------|
| `"tests"` | `tests/unit/test_foo.py` | Yes | Component match (no `/` in pattern) |
| `"tests"` | `src/docvet/test_utils.py` | No | `test_utils` != `tests` |
| `"scripts"` | `scripts/gen.py` | Yes | Component match |
| `"*.pyc"` | `src/docvet/__pycache__/foo.pyc` | Yes | Filename glob match (no `/`) |
...
- [ ] Task 1: Create `src/docvet/discovery.py` with `DiscoveryMode` enum and private helpers
  ...
  - Action: Create the module with:
    ...
    - Imports: `enum`, `fnmatch`, `subprocess`, `sys` from stdlib; `Path` from `pathlib`; `DocvetConfig` from `docvet.config`
    ...
    - `_is_excluded(rel_path: str, exclude: list[str]) -> bool` — For each pattern in `exclude`: if pattern contains `/`, match against full `rel_path` with `fnmatch.fnmatch`; if pattern has no `/`, match against each component of `PurePosixPath(rel_path).parts` with `fnmatch.fnmatch`. Return `True` on first match.
    - `_walk_all(config: DocvetConfig) -> list[Path]` — Resolve `root = config.project_root / config.src_root`. If `root` doesn't exist: warn `"docvet: src-root '{config.src_root}' not found"` to stderr → return `[]`. Try `git ls-files --cached --others --exclude-standard -- '*.py'` with `cwd=root` and `warn=False` (suppress git failure warning since we fall back silently). If git succeeds: resolve each line to absolute path, filter `.py`, filter `_is_excluded`, skip symlinks. If git fails (non-git directory): fallback to iterating `root.rglob("*.py")`, skip symlinks via `path.is_symlink()`, filter `_is_excluded` using path relative to `config.project_root`.
  - Notes: Use `# ---------------------------------------------------------------------------` section separators between logical blocks (Enums, Private helpers, Public API). All functions get full Google-style docstrings.

- [ ] Task 2: Implement `discover_files()` public function
  - File: `src/docvet/discovery.py`
  - Action: Add the public function:
    - Signature: `def discover_files(config: DocvetConfig, mode: DiscoveryMode, *, explicit_files: list[Path] | None = None) -> list[Path]`
    - DIFF mode: `_run_git(["diff", "--name-only", "--diff-filter=ACMR"], cwd=config.project_root)` → filter `.py` → filter `_is_excluded` → resolve to absolute paths
    - STAGED mode: `_run_git(["diff", "--cached", "--name-only", "--diff-filter=ACMR"], cwd=config.project_root)` → same filtering pipeline as DIFF
    - ALL mode: delegate to `_walk_all(config)`
    - FILES mode: iterate `explicit_files`, for each: check `.suffix == ".py"` (skip non-Python), check `.exists()` (warn + skip if missing: `print(f"docvet: file not found: {path}", file=sys.stderr)`), resolve to absolute. Do NOT apply `_is_excluded` (explicit files bypass excludes).
    - All modes: return `sorted(paths)` for determinism
  - Notes: The `explicit_files` parameter is keyword-only. If `mode is DiscoveryMode.FILES` and `explicit_files is None`, return `[]`.

- [ ] Task 3: Update `src/docvet/cli.py` — remove `DiscoveryMode`, import from `discovery`
  - File: `src/docvet/cli.py`
  - Action:
    - Remove the `DiscoveryMode` class definition (lines 16-23)
...
- [ ] Task 5: Create `tests/unit/test_discovery.py` — unit tests
  ...
  - Action: Create unit test file with:
    ...
    - **`discover_files` FILES mode tests**:
      ...
      - `test_discover_files_files_mode_when_none_returns_empty` — `explicit_files=None` → `[]`
    - **General tests**:
  ...
  - Notes: Use `mocker.patch("docvet.discovery.subprocess.run")` for mocking. Use `capsys` for stderr assertions. Use `tmp_path` for filesystem trees. Follow `test_<what>_when_<condition>_<expected>` naming.
...
- [ ] Task 7: Update project documentation
  ...
  - Action:
    ...
    - `docs/architecture.md`: Change `discovery.py (Planned)` to `discovery.py (Implemented)`. Update the module description to reflect the actual implementation: four modes, `git ls-files` for ALL mode, `.gitignore`-semantic excludes, `DiscoveryMode` enum lives here.
...
- [ ] AC 28: Given a git repo with a staged `.py` file in an excluded directory, when `discover_files` is called with `DiscoveryMode.STAGED`, then the file is NOT in the result.

**ALL mode:**
- [ ] AC 14: Given a git repo with tracked `.py` files, when `discover_files` is called with `DiscoveryMode.ALL`, then tracked files are returned.
- [ ] AC 15: Given a git repo with a `.gitignore`d `.py` file, when `discover_files` is called with `DiscoveryMode.ALL`, then the gitignored file is NOT in the result.
- [ ] AC 16: Given a non-git directory with `.py` files, when `discover_files` is called with `DiscoveryMode.ALL`, then files are returned via `rglob` fallback.
- [ ] AC 17: Given `src_root` that doesn't exist, when `discover_files` is called with `DiscoveryMode.ALL`, then result is `[]` and a warning is printed to stderr.
- [ ] AC 18: Given a non-git directory with a symlink `.py` file, when `discover_files` is called with `DiscoveryMode.ALL`, then the symlink is NOT in the result.
- [ ] AC 19: Given a non-git directory with files in an excluded dir, when `discover_files` is called with `DiscoveryMode.ALL`, then excluded files are NOT in the result.

**FILES mode:**
- [ ] AC 20: Given a list of existing `.py` files, when `discover_files` is called with `DiscoveryMode.FILES`, then all files are returned.
- [ ] AC 21: Given a list containing a non-existent file, when `discover_files` is called with `DiscoveryMode.FILES`, then the missing file is skipped and a warning is printed to stderr.
- [ ] AC 22: Given a list containing a `.py` file in an excluded directory, when `discover_files` is called with `DiscoveryMode.FILES`, then the file IS returned (explicit files bypass excludes).
- [ ] AC 23: Given a list containing a non-`.py` file, when `discover_files` is called with `DiscoveryMode.FILES`, then the non-Python file is NOT returned.
- [ ] AC 24: Given `explicit_files=None` with `DiscoveryMode.FILES`, when `discover_files` is called, then result is `[]`.

**General:**
- [ ] AC 25: Given any discovery mode with results, when `discover_files` returns, then all paths are absolute and the list is sorted.
- [ ] AC 26: Given `DiscoveryMode.DIFF` in a non-git directory, when `discover_files` is called, then result is `[]` and a warning is printed to stderr.

**Enum migration:**
- [ ] AC 27: Given the updated `cli.py` importing `DiscoveryMode` from `discovery`, when all existing CLI tests run, then they pass without modification.

## Additional Context

### Dependencies

- No new dependencies — all stdlib (`subprocess`, `pathlib`, `fnmatch`, `sys`, `enum`)
...
- **Unit tests** (`tests/unit/test_discovery.py`):
  ...
  - General: sorted absolute paths, empty results (2 test cases)
  - Total: ~22 unit tests
...
- **Integration tests** (`tests/integration/test_discovery.py`):
  - DIFF mode: modified file, no changes, non-py filtering, exclude patterns (4 test cases)
  - STAGED mode: staged file, nothing staged, exclude patterns (3 test cases)
  - ALL mode: tracked files, gitignored files, excludes on top of git (3 test cases)
